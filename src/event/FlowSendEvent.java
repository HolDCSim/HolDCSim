 ...
 * 
 */
public final class FlowSendEvent extends AbstractEvent {
	...
	public FlowSendEvent(final double time, final Experiment experiment,
...
//						+ "] is sent from port " + portId + " of " + node.getNodeTypeName() + " " 
//						+ node.getNodeId() + ". Job "+ flow.getJob().getJobId()+", task "
//						+ flow.getSourceTask().getTaskId() +" to " + flow.getDestinationTask().getTaskId() 
//						+ ".");
//			} else if (node.getNodeTypeName() == "switch") {
//				System.out.println("Time: " + this.getTime() + ", [FLOW_EVENT], flow for [server#" 
//						+ this.flow.getSourceNode().getNodeId()+ " --> server#" + this.flow.getDestinationNode().getNodeId() 
//						+ "] is sent from port " + portId + " of linecard " + linecardId 
//						+ " in " + node.getNodeTypeName() + " " + node.getNodeId() + ". Job "
//						+ flow.getJob().getJobId()+", task "+ flow.getSourceTask().getTaskId() +" to " 
//						+ flow.getDestinationTask().getTaskId() + ".");
//			}
		}
	...
	public void process() {
		...
		if (flow.getCurrentHop() == 0 || flow.isLastHopIngress()) {
